...
#'
#' @return input A list showing input parameters to the function
#' @return Design A list with two elements: \code{design} and \code{BlocksEfficiency}. \code{design} is a data frame giving lane and adaptor assignment for treatment groups. \code{BlocksEfficiency} is a data frame giving block efficiencies (D-Efficiencies) for lane and adaptor.
#' @return suggestedDesign When \code{nperlane} is \code{NULL} or same as the value would be selected to give better efficiencey, \code{suggestedDesign} is the same as \code{Design}. Otherwise, \code{suggestedDesign} uses different number of samples per lane from the input to give more efficient block design. \code{suggestedDesign} is a list with two elements: \code{design} and \code{BlocksEfficiency}. \code{design} is a data frame giving lane and adaptor assignment for treatment groups. \code{BlocksEfficiency} is a data frame giving block efficiencies (D-Efficiencies) for lane and adaptor.
#'
#'
...
#'
#' @examples
#' gen.design(trts=letters[1:4], reps=rep(4,4), nperlane=NULL)
#' gen.design(trts=letters[1:4], reps=c(2,4,4,2), nperlane=3)
#' gen.design(trts=letters[1:6], reps=rep(3,6), nperlane=4)
#'
#' @import dplyr
